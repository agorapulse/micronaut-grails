 ...
 */
package org.grails.compiler.boot.micronaut
 ...
 * extends {@link MicronautGrailsAutoConfiguration}, it produces:
 *
 * <pre>
 * <code>
 *
 * class ApplicationLoader extends MicronautGrailsAppServletInitializer {
 *
...
class BootInitializerClassInjector extends GlobalClassInjectorAdapter {
    ...
    public static final ClassNode GRAILS_CONFIGURATION_CLASS_NODE = ClassHelper.make(MicronautGrailsAutoConfiguration)
        ...
        if(GrailsASTUtils.isAssignableFrom(GRAILS_CONFIGURATION_CLASS_NODE, classNode) && !GrailsASTUtils.isSubclassOfOrImplementsInterface(classNode, GrailsPluginApplication.name)) {
            ...
            for(MethodNode mn in methods) {
                if(Modifier.isStatic(mn.modifiers) && Modifier.isPublic(mn.modifiers)) {
                    ...
                    def loaderClassNode = new ClassNode("${classNode.name}Loader", Modifier.PUBLIC, ClassHelper.make(MicronautGrailsAppServletInitializer))
